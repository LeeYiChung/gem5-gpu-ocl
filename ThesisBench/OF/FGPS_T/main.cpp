...
void opticalFlow(cv::Mat& srcImage1, cv::Mat& srcImage2, cv::Size window, int iteration) {
    ...
    /*for(int y = 0; y < srcImage1.rows; y++) {
        for(int x = 0; x < srcImage1.cols; x++) {
            arr1d1[y * srcImage1.cols + x] = srcImage1.at<uchar>(y, x);
            arr1d2[y * srcImage1.cols + x] = srcImage2.at<uchar>(y, x);
        }
    }*/
        ...
        for(int bx = 0; bx < hCols/BLOCK_SIZE; bx++) {
            for(int ty = 0; ty < BLOCK_SIZE; ty++) {
                ...
                for(int tx = 0; tx < BLOCK_SIZE; tx++) { 
                    ...
                    arr1d1[yidx * hCols + xidx] = hImage1[yidx][xidx];
                    arr1d2[yidx * hCols + xidx] = hImage2[yidx][xidx];
                }
            }
